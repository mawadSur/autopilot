...
try:
    ...
    # Define a placeholder function that returns nothing if the real one is missing.
...
def compute_technical_indicators(df):
    """
    Calculates all features for the entire DataFrame at once (vectorized).
    """
    ...
    try:
        ...
        print(f"Could not calculate hourly trend features: {e}. Skipping.")
        df['hourly_ema_20'] = 0
...
def run_backtest_with_capital(df, signals, initial_capital=10000):
    """
    Runs the backtest with a starting capital, tracking equity over time and using
    real entry/exit prices for PNL calculation.
    """
    print("\n📈 Starting backtest with initial capital...")
    TRADING_FEE_PCT = 0.1 # More realistic 0.1% fee
    TAKE_PROFIT_PCT = 2.0
    STOP_LOSS_PCT = 0.50
    TRADE_AMOUNT_PCT = 0.50 # Use 50% of capital for each trade
    ...
    trades = []
    winning_trades = 0
    trade_entries = [] # To store entry points for plotting
    ...
    for i in tqdm(range(len(df)), desc="Backtesting"):
        ...
        # Forward-fill equity curve with the last known capital value
        ...
        if in_position:
            ...
            if exit_price is not None:
                # Calculate PNL based on actual prices
                gross_pnl = position_size * ((exit_price / entry_price) - 1)
                
                # Subtract fees based on dollar amounts
                entry_fee = position_size * (TRADING_FEE_PCT / 100)
                exit_fee = (position_size + gross_pnl) * (TRADING_FEE_PCT / 100)
                net_pnl = gross_pnl - entry_fee - exit_fee

                # Update capital and log the trade
                capital += net_pnl
                trades.append(net_pnl)
                in_position = False

                # Update equity curve for the entire period of the trade
        ...
        if not in_position and signals[i] == 1:
            ...
            # Risk a percentage of current capital
            position_size = capital * TRADE_AMOUNT_PCT 
            take_profit_price = entry_price * (1 + TAKE_PROFIT_PCT / 100)
            stop_loss_price = entry_price * (1 - STOP_LOSS_PCT / 100)
            # Log index of trade entry for plotting
...
def plot_backtest_results(df, equity_curve, trade_entries):
    """
    Plots the price, equity curve, and trade entry points.
    """
    ...
    # Create a second y-axis for the equity curve
    ax2 = ax1.twinx()
...
def main():
    ...
    MODEL_DIR = "."
    # Create dummy model artifacts if they don't exist to prevent errors
    ...
    if not os.path.exists(scaler_path):
        ...
        dummy_scaler.fit(np.random.rand(100, model_config['input_size']))
        joblib.dump(dummy_scaler, scaler_path)

    if not os.path.exists(model_path):
         dummy_model = LSTMModel(**{k: v for k, v in model_config.items() if k != 'window_size'})
         torch.save(dummy_model.state_dict(), model_path)

    print("Loading historical data...")
    try:
        # Try loading real data first
        chunks = load_ohlc_chunks(folder='eth_1m_data', chunk_mode=True)
        if not chunks:
            print("No data chunks found. Please check 'eth_1m_data' folder.")
    ...
    if df_full.empty:
        print("No data loaded. Cannot proceed with backtest.")
        return

    # --- FIX for FutureWarning and robust date conversion ---
    # Explicitly convert to numeric, coercing errors to NaT (Not a Time)
    ...
    # --- NEW FIX: Aggregate duplicate timestamps instead of just dropping ---
    if df_full.index.has_duplicates:
        print(f"Found {df_full.index.duplicated().sum()} duplicate timestamps. Aggregating them...")
        df_full = df_full.groupby(df_full.index).agg({
            'open': 'first',
            'high': 'max',
            'low': 'min',
            'close': 'last',
    ...
    # Create a complete date range from the first to the last timestamp with 1-minute frequency
    full_range_index = pd.date_range(start=df_full.index.min(), end=df_full.index.max(), freq='T')
    # Reindex the dataframe. This will create NaNs for missing timestamps.
    df_resampled = df_full.reindex(full_range_index)
    # Forward-fill the missing values.
    df_resampled.ffill(inplace=True)


    # Use a recent chunk of data for the backtest
    df = df_resampled.iloc[-50000:].copy() 
    if df.empty:
        print("No data available for backtesting after processing.")
        return

    # --- NEW FIX: Check for sufficient data length BEFORE feature calculation ---
    # The longest rolling window for indicators is 50. We need at least that many rows.
    LONGEST_INDICATOR_WINDOW = 50
    if len(df) < LONGEST_INDICATOR_WINDOW:
        print(f"Error: DataFrame has only {len(df)} rows after cleaning.")
        print(f"A minimum of {LONGEST_INDICATOR_WINDOW} rows is required to compute all technical indicators.")
        return

    print("1. Computing features for the entire dataset...")
    df_features = compute_technical_indicators(df.copy())
    
    # Final check to ensure dataframe is not empty after feature calculation
    if df_features.empty:
        print("Error: DataFrame is empty after feature calculation and dropping NaNs.")
        print("This can happen if the dataset is too small to compute rolling indicators (e.g., 50-period SMA).")
        return
    ...
    # --- FIX: Ensure we have enough data for the model window ---
    window_size = model_config.get('window_size', 60)
    if len(df_features) < window_size:
        print(f"Error: Not enough data for model prediction. Have {len(df_features)} rows, need {window_size}.")
        return
