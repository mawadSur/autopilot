...
def _apply_default_headers(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Header normalization must stay schema-only. All multi-timeframe feature
    # engineering happens in compute_features(), where the resampled signals are
    # explicitly shifted by one completed TF bar before merge.
    return df
