...
SYSTEM_PROMPT_TEMPLATE = (
    "You are a Twitter/X research analyst for event-driven prediction markets. Your job is to "
    "read short-form social posts and replies, separate signal from noise, and estimate how "
    "Twitter discourse should update the probability for the market: {market_title}. "
    "Current market implied probability is {implied_probability_pct}%. "
    "Weight tweets from credentialed analysts, primary-source links, and threads with "
    "informed disagreement above viral hot-takes, recycled memes, or astroturfing.\n\n"
    "Return ONLY a JSON object that matches the required TwitterResearchReport schema with "
    ...
    "- evidence_quality_score: int 0-100; higher = stronger primary-source coverage.\n"
    "- misinformation_risk_score: int 0-100; higher = higher misinformation/manipulation risk.\n"
    "- sentiment_score: int -100..100; negative=bearish, positive=bullish, 0=neutral.\n"
    "- key_sources: list of up to 10 tweet URLs or @handles cited as the strongest references.\n"
    "- summary: non-empty narrative summary of the Twitter/X discussion landscape.\n"
    "- tweet_count: int >= 0; how many tweets your analysis pulled from."
)
...
def _coerce_report(parsed: Any) -> TwitterResearchReport:
    if isinstance(parsed, TwitterResearchReport):
        return parsed
    if isinstance(parsed, dict):
        return TwitterResearchReport.model_validate(parsed)
    if isinstance(parsed, str):
        return TwitterResearchReport.model_validate_json(parsed)
    raise TypeError(f"Unsupported parsed response type: {type(parsed)!r}")


def _parse_report_response(response: Any) -> TwitterResearchReport:
    ...
    try:
        return TwitterResearchReport.model_validate_json(cleaned)
    except Exception:
        start = cleaned.find("{")
        end = cleaned.rfind("}")
        ...
        return TwitterResearchReport.model_validate(payload)
...
class TwitterAgent:
    ...
    async def analyze_discussion(
        ...
        **_: Any,
        ...
        context = (twitter_context or "").strip()
        implied_probability = float(implied_prob)

        if not title:
            raise ValueError("market_title must be a non-empty string")
        if not context:
            raise ValueError("twitter_context must be a non-empty string")
        if not 0.0 <= implied_probability <= 1.0:
            raise ValueError("implied_prob must be between 0.0 and 1.0")

        prompt = (
            "Analyze the following Twitter Discussion Context and return only a JSON object that "
            "matches the required TwitterResearchReport schema with ALL fields populated.\n\n"
    ...
    def _generation_config(self, market_title: str, implied_prob: float) -> Any:
        return self._types.GenerateContentConfig(
            ...
            response_schema=TwitterResearchReport,
            temperature=0.2,
