...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if (
        not symbols
        and not polymarket_market_ids
        and not hyperliquid_symbols
        and not alpaca_symbols
    ):
        # Preserve the legacy error string so existing CLI consumers /
        # tests that key off the substring "--symbols must contain at
        # least one entry" continue to work. We additionally surface the
        # polymarket / hyperliquid / alpaca options for operators who
        # want the union.
        print(
            "error: --symbols must contain at least one entry "
            "(or pass --polymarket-markets / --hyperliquid-symbols / --alpaca-symbols)",
            file=sys.stderr,
        )
        return 2

    # Build PolymarketTradeable / HyperliquidTradeable / AlpacaTradeable
    # instances up-front so SupervisorConfig validation sees the union of
    # (symbols, tradeables) and the supervisor's __init__ wires them into
    # the iteration list.
    tradeables: List[Any] = []
    ...
    # Include polymarket / hyperliquid / alpaca ids in the run-dir name
